...
export default function RecordPaymentModal({ purchaseOrder, onClose, onSuccess }: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`/api/purchase-orders/${purchaseOrder.id}/payments`, {
        method: 'POST',
      ...
      if (result.success) {
        toast.success(`Payment recorded successfully! Remaining: ₹${result.data.remainingAmount.toLocaleString('en-IN')}`, 5000);
        onSuccess();
      } else {
  ...
  return (
    <div style={{
      ...
      <div className="card" style={{ maxWidth: '500px', width: '100%', maxHeight: '90vh', overflowY: 'auto' }}>
        <h2 style={{ marginTop: 0, borderBottom: '2px solid #f59e0b', paddingBottom: '10px' }}>
          💳 Record Payment
        </h2>
        ...
        <div style={{ marginBottom: '20px', padding: '12px', background: '#f3f4f6', borderRadius: '4px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
            ...
            <strong>₹{Number(purchaseOrder.totalAmount).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</strong>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', color: '#28a745' }}>
            <span>Paid Amount:</span>
            <strong>₹{Number(purchaseOrder.paidAmount).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</strong>
          </div>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            <button
              ...
              style={{
                flex: 1,
                padding: '12px 20px',
                background: submitting ? '#9ca3af' : '#f59e0b',
                color: 'white',
                border: 'none',
                borderRadius: '4px',
            ...
            >
              {submitting ? 'Recording...' : 'Record Payment & Create Expense'}
            </button>
            <button
              type="button"
              onClick={onClose}
